...
namespace InfSystBusStation {
    ...
    public:
        // Конструкторы без аргументов по умолчанию
        ResultForm(TripList^ tripList, BusList^ busList, DriversList^ driverList)
            : ResultForm(tripList, busList, driverList, false) {
        } // Делегирующий конструктор

        ResultForm(TripList^ tripList, BusList^ busList, DriversList^ driverList, bool isAdminMode) {
            InitializeComponent();
